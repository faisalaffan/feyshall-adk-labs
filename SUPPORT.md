# Support

## Documentation

- **[README.md](README.md)** — project overview and getting started
- **[PRD](docs/PRD.md)** — full roadmap and chapter breakdown
- **[CONTRIBUTING.md](CONTRIBUTING.md)** — how to contribute recipes

## Getting Help

- **Questions?** Open a [GitHub Discussion](https://github.com/feyshall/feyshall-adk-labs/discussions)
- **Bug in a recipe?** Open an [issue](https://github.com/feyshall/feyshall-adk-labs/issues/new?template=bug_report.md)
- **Request a recipe?** Open a [feature request](https://github.com/feyshall/feyshall-adk-labs/issues/new?template=feature_request.md)

## Priority Support

This is a community-maintained cookbook. There is no SLA. For urgent queries, try the [official ADK documentation](https://google.github.io/adk-docs/) or the [ADK GitHub Discussions](https://github.com/google/adk-docs/discussions).
